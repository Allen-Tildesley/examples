...
PROGRAM initialize
  ...
  INTEGER            :: molecule_option ! User option for atoms, linear, or nonlinear molecule
  INTEGER, PARAMETER :: atoms = 0, linear = 1, nonlinear = 2
  ...
  NAMELIST /nml/ nc, n, temperature, inertia, density, length, velocities, molecules, lattice, soft
  ...
  WRITE ( unit=output_unit, fmt='(a)' ) 'Options for molecules are "atoms", "linear", "nonlinear"'
